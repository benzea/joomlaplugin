...
"""This is a slightly modified version of the authz_policy to implement
getting group names out of Joomla. So in addition to reading the [groups]
the Joomla groups are loaded.
...
class JoomlaAuthzPolicy(Component):
	...
	def get_user_groups(self, username):
		...
		for group in JoomlaACL(self.env).get_user_groups(name=username):
			groups.add('@' + group)
		return groups
			...
			if fnmatch(resource_key, resource_glob):
				...
				for who, permissions in section.iteritems():
					if who in valid_users or \
							who in self.get_user_groups(username):
